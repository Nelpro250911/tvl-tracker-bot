...
CHAT_ID = os.getenv("CHAT_ID", "1900314873")
...
with open(os.path.join(backup_dir, 'backup_log.txt'), 'a') as log:
    log.write(log_entry)

# Уведомление о бэкапе
requests.post(f'https://api.telegram.org/bot{API_TOKEN}/sendMessage', data={
    'chat_id': CHAT_ID,
    'text': f"✅ Бэкап создан: {backup_file}"
})
